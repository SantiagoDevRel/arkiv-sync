...
- **Write throughput ceiling ≈ 25 events/sec** — one wallet (single nonce) × ~50 entities per `mutateEntities` tx × Braga ~2s blocks. A contract sustaining more than that lags (it never drops/crashes; the lag is logged). Plenty for low/moderate-traffic contracts.
  - **Scaling path (designed, not built in v1): a multi-wallet pool.** N funded wallets = N parallel nonce lanes ≈ **N × 25 ev/s** (scales ~linearly). Assign each event to a wallet by `hash(eventId) % N` so the same event always lands on the same wallet → idempotency + reorg reconciliation stay correct (each wallet owns its slice and never double-writes another's). The only operational cost is funding N burners. Build it when you need to index a genuinely high-volume mainnet contract in real time.
